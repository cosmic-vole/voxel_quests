		...
		if type(talk) ~= 'table' then
			...
			if evaluate_condition(pname, pcontext, cond) then
				...
				elseif talk.who == NPC then
					lines = lines + 1
					table.insert(npctalk, talk) --!!! TODO need to associate the right talk object with the chosen line!!! save both of them here! or just choose at point of printing out
				end
			end
...
--	)
	...
	elseif pcontext.npcdef.convos.Talk or pcontext.npcdef.convos.Rumors then --and convo_cname == "Talk" then
		local markup =  "hypertext[0,2.2;12,4;talk;" --"textlist[0,3.5;8,2.5;talk;"--"hypertext[0,2.2;8,4;talk;" --  "label[0,2.4;"
		local firsttalk = nil
...
local function register_npcmob(name, def)
	...
	--collisionbox = {-0.45, -0.5, -0.25, 0.45, 1.45, 0.25},
	mesh = "player.b3d",
...
--]]
	...
	attack={
		...
		on_punch = function(self, puncher, time_from_last_punch, tool_capabilities, dir, damage)--(pos, node, puncher, ...)
			--if def.on_punch and def.on_punch(pos, node, puncher, ...) then
			--	return
			--end
					minetest.log("action",
			("player '%s' punches npcmob '%s'"):format(puncher:get_player_name() or "!", def.npcname) --puncher:get_player_name() or "!"
		)
		...
		on_rightclick = function(self, clicker)--(pos, node, clicker, itemstack, pointed_thing)
			minetest.log("action",
			("right click on npcmob '%s'"):format(def.npcname)) --puncher:get_player_name() or "!"
			if not clicker or not clicker:is_player() then return end
...
--The follower entity should really only be added when the relevant area of the map is generated by the player moving there
--local pos = vector.new(138,9,-14)
--local offset = vector.new(2,2,2)
--minetest.load_area(vector.subtract(pos, offset), vector.add(pos, offset))
--minetest.add_entity(pos, modname..":follower1")

-- register_overworld_spawner(entity, enemies_per_spawner, biomes, func, fill_ratio)
--spawners.register_overworld_spawner(modname..":follower1", 1, nil, nil, 0.00003)
